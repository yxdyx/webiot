layui.config({
	...
	$.get("../../json/newsList.json", function(data){
		var newArray = [];
		//单击首页“待审核用户”加载的信息
		if($(".top_tab li.layui-this cite",parent.document).text() == "待审核用户"){
	...
	$(".audit_btn").click(function(){
		...
		if($checkbox.is(":checked")){
			var index = layer.msg('更改中，请稍候',{icon: 16,time:false,shade:0.8});
            setTimeout(function(){
            	for(var j=0;j<$checked.length;j++){
            		for(var i=0;i<newsData.length;i++){
						if(newsData[i].newsId == $checked.eq(j).parents("tr").find(".news_del").attr("data-id")){
							//修改列表中的文字
							$checked.eq(j).parents("tr").find("td:eq(4)").text("在线").removeAttr("style");
							//将选中状态删除
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
					...
					+'<td>'+currData[i].newsPwd+'</td>'
			    	+'<td>'+currData[i].newsAuthor+'</td>';
			    	if(currData[i].newsStatus == "离线"){
